...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  download: (
    ...
    ipcRenderer.invoke("ftp:download", {
      remotePath,
      fileName,
      downloadId,
      totalSize,
  ...
  deriveKeyFromPassword: (password: string, version: string, exportDate: string) =>
    ipcRenderer.invoke("encryption:derive-key", { password, version, exportDate }),
  encryptField: (text: string, keyHex: string) => ipcRenderer.invoke("encryption:encrypt-field", { text, keyHex }),
  decryptField: (encryptedText: string, keyHex: string) => ipcRenderer.invoke("encryption:decrypt-field", { encryptedText, keyHex }),
  encryptSite: (site: any, keyHex: string) => ipcRenderer.invoke("encryption:encrypt-site", { site, keyHex }),
  decryptSite: (site: any, keyHex: string) => ipcRenderer.invoke("encryption:decrypt-site", { site, keyHex }),
  ...
  deleteMultiple: (filePaths: string[]) => ipcRenderer.invoke("ftp:delete-multiple", { filePaths }),
  renameEntry: (oldPath: string, newPath: string) => ipcRenderer.invoke("ftp:rename", { oldPath, newPath }),
  ...
  collectFolderFiles: (folderPath: string, baseRemotePath: string) =>
    ipcRenderer.invoke("fs:collect-folder-files", { folderPath, baseRemotePath }),
  ...
  downloadFolder: (
    ...
    ipcRenderer.invoke("ftp:download-folder", {
      remotePath,
      folderName,
      downloadId,
      defaultDownloadPath,
  ...
  collectLocalEntries: (localPath: string) => ipcRenderer.invoke("upload:collect-local-entries", localPath),

  // Connection status
  onConnectionLost: (callback: (data: { reason: string; code?: string }) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, data: any) => callback(data);
    ipcRenderer.on("connection:lost", handler);
    return () => ipcRenderer.removeListener("connection:lost", handler);
  },
