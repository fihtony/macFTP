...
import { TEXT_EXTENSIONS, IMAGE_EXTENSIONS } from "../../shared/fileExtensions";
...
export const startKeepAlive = () => {
  ...
  keepAliveTimer = setInterval(async () => {
    try {
      ...
      // Check if this is a connection error (ETIMEDOUT, ERR_NOT_CONNECTED, etc.)
      ...
      if (isConnectionError) {
        console.log("[Keep-Alive] Connection lost, force disconnecting...");
        stopKeepAlive();

        // Force disconnect without waiting (connection is already dead)
        // Don't call end() or close() as they may hang on broken connections
        ...
        windows.forEach((win) => {
          win.webContents.send("connection:lost", {
            reason: "Connection timeout or lost",
            code: err.code,
          });
        });
      }
  ...
  try {
    ...
    if (config.protocol === "sftp") {
      ...
      if (config.privateKeyContent) {
        ...
        sftpOptions.privateKey = config.privateKeyContent;
      } else if (config.privateKeyPath) {
        // Use key file path
        try {
          const expandedPath = config.privateKeyPath.startsWith("~")
            ? path.join(os.homedir(), config.privateKeyPath.slice(1))
            : config.privateKeyPath;

          if (fs.existsSync(expandedPath)) {
            sftpOptions.privateKey = fs.readFileSync(expandedPath);
          } else {
            throw new Error(`SSH key file not found: ${config.privateKeyPath}`);
          }
        } catch (err: any) {
        ...
        sftpOptions.password = config.password;
      } else {
        throw new Error("No authentication method provided (SSH key or password required)");
      }

      await sftpClient.connect(sftpOptions);
    } else {
...
// Batch delete multiple files (files only, not directories)
ipcMain.handle("ftp:delete-multiple", async (_event, { filePaths }: { filePaths: string[] }) => {
  const results: Array<{ path: string; success: boolean; error?: string }> = [];

  try {
    if (!currentProtocol || (!sftpClient && !ftpClient)) {
      throw new Error("Not connected");
    }
    ...
    for (const filePath of filePaths) {
      try {
        if (currentProtocol === "sftp" && sftpClient) {
          await sftpClient.delete(filePath);
        } else if (currentProtocol === "ftp" && ftpClient) {
          await ftpClient.remove(filePath);
        }
        results.push({ path: filePath, success: true });
      } catch (err: any) {
        console.error("[Batch Delete] Failed to delete:", filePath, err.message);
    ...
    return {
      success: failCount === 0,
      results,
      summary: { total: filePaths.length, success: successCount, failed: failCount },
    };
  } catch (err: any) {
...
ipcMain.handle("ftp:rename", async (_event, { oldPath, newPath }: { oldPath: string; newPath: string }) => {
  try {
    if (currentProtocol === "sftp" && sftpClient) {
...
ipcMain.handle(
  "fs:collect-folder-files",
  async (_event, { folderPath, baseRemotePath }: { folderPath: string; baseRemotePath: string }) => {
...
ipcMain.handle("ftp:preview-file", async (event, { remotePath, fileName }: { remotePath: string; fileName: string }) => {
  try {
    ...
    const isText = TEXT_EXTENSIONS.includes(ext as any);
    const isImage = IMAGE_EXTENSIONS.includes(ext as any);
...
ipcMain.handle("ftp:get-path-suggestions", async (event, targetPath: string) => {
  try {
    ...
    const searchTerm = targetPath.includes("/")
...
let startFolderDownloadCallback:
  | ((
      id: string,
      remotePath: string,
      localPath: string,
...
export const setStartFolderDownloadCallback = (
  callback: (
    id: string,
    remotePath: string,
    localPath: string,
  ...
  while (unifiedDownloadQueue.length > 0) {
    ...
    if (notifyDownloadProgressCallback) {
      notifyDownloadProgressCallback({
        ...
        localPath:
          item.type === "file"
            ? item.fileJob?.job.localPath
            : item.folderJob?.localPath
            ? path.join(item.folderJob.localPath, item.folderJob.folderName)
            : undefined,
        endTime: Date.now(),
      });
    }
  ...
  if (item.type === "file" && item.fileJob && startFileDownloadCallback) {
    ...
    startFileDownloadCallback(item)
      ...
      .finally(() => {
        ...
        console.log(
          "[Unified Queue] File download finished:",
          item.id,
          "Active:",
          totalActiveDownloads,
          "Queue:",
          unifiedDownloadQueue.length
        );
        // Process queue immediately and then again with a small delay to catch any race conditions
        processUnifiedQueue();
        setTimeout(() => processUnifiedQueue(), 50);
      });
  } else if (item.type === "folder" && item.folderJob && startFolderDownloadCallback) {
    // Process folder download
    console.log(
      "[Unified Queue] Starting folder download:",
      item.id,
      "Active:",
      totalActiveDownloads,
      "Queue:",
    ...
    startFolderDownloadCallback(
      item.folderJob.id,
      item.folderJob.remotePath,
      item.folderJob.localPath,
      item.folderJob.folderName,
      item.folderJob.defaultConflictResolution
    )
      .catch((err) => {
      ...
      .finally(() => {
        totalActiveDownloads = Math.max(0, totalActiveDownloads - 1); // Ensure counter never goes negative
        console.log(
          "[Unified Queue] Folder download finished:",
          item.id,
          "Active:",
          totalActiveDownloads,
          "Queue:",
  ...
  if (fs.existsSync(defaultPath)) {
    if (finalDuplicateAction && finalApplyToAll) {
      ...
      const result = await dialog.showMessageBox(win, {
        ...
        checkboxLabel: showApplyToAll ? "Apply to all similar cases" : undefined,
...
export const handleDuplicateUploadFile = async (
  ...
  if (existsResult.success && existsResult.exists) {
    ...
    // If duplicateAction is provided (session resolution applied), use it directly
    if (finalDuplicateAction && !finalApplyToAll) {
      // Session resolution exists but "apply to all" was not checked
      // This means we should still use the resolution for this file only
      console.log("[Duplicate Upload] Using session conflict resolution (not applied to all):", finalDuplicateAction);
      if (finalDuplicateAction === "skip") {
      ...
      // Use global default conflict resolution setting when it's not 'prompt' or 'ask'
      console.log("[Duplicate Upload] Using default conflict resolution:", defaultConflictResolution);
      if (defaultConflictResolution === "skip") {
        return { remotePath: "", actualFileName: fileName, duplicateAction: "skip", applyToAll: false, skipped: true };
      } else if (defaultConflictResolution === "overwrite") {
        finalRemotePath = remotePath;
        finalDuplicateAction = "overwrite";
      } else if (defaultConflictResolution === "rename") {
        // Rename: generate unique remote filename
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      const result = await handleDuplicateUploadFile(
        win,
        remotePath,
        fileName,
        duplicateAction,
...
// IPC handler for collecting local folder entries
ipcMain.handle("upload:collect-local-entries", async (event, localPath: string) => {
  try {
    ...
    console.error("[Upload] Error collecting local entries:", err);
    return { success: false, error: err.message };
  }
});
