...
const getEncryptionKeySync = (): Buffer => {
  ...
  const appData = app.getPath("userData") + app.getName();
  const machineData = [os.homedir(), os.hostname(), os.type(), process.platform].join("");
  const keyMaterial = appData + machineData;
...
export const saveSites = (sites: Site[]): void => {
  // Encrypt sensitive fields: host, username, password, private key path and content
  ...
  const encryptedSites = sites.map((site) => ({
    ...
    privateKeyPath: site.privateKeyPath ? encrypt(site.privateKeyPath) : undefined,
...
export const loadSites = (): Site[] => {
  ...
  return encryptedSites.map((site: any) => {
    ...
    let decryptedPrivateKeyPath = site.privateKeyPath;
    ...
    // Decrypt all sensitive fields (host, user, password, privateKeyPath, privateKeyContent)
    ...
    decryptedPrivateKeyPath = tryDecrypt(site.privateKeyPath, "privateKeyPath");
    ...
    return {
      ...
      privateKeyPath: decryptedPrivateKeyPath || undefined,
...
export const cleanupCorruptedData = (): void => {
  ...
  const cleanedSites = encryptedSites.map((site: any) => {
    ...
    // Check all encrypted fields (host, user, password, privateKeyPath, privateKeyContent)
    ...
    checkAndClean(site.privateKeyPath, "privateKeyPath");
...
export const saveDownloadHistory = (downloads: DownloadHistoryItem[]): void => {
  ...
  const historyItems = downloads.filter((d) => d.status === "completed" || d.status === "failed" || d.status === "cancelled").slice(-1000); // Keep last 1000 items
...
export const saveAllDownloads = (downloads: DownloadHistoryItem[]): void => {
  ...
  const activeDownloads = downloads.filter((d) => d.status === "queued" || d.status === "downloading" || d.status === "paused");
  const historyDownloads = downloads.filter((d) => d.status === "completed" || d.status === "failed" || d.status === "cancelled");

  // Sort history by endTime descending (newest first) and take last 1000
  // slice(-1000) keeps the most recent 1000 items
  const sortedHistory = historyDownloads.sort((a, b) => (b.endTime || 0) - (a.endTime || 0)).slice(-1000);
...
export const loadAllDownloads = (): DownloadHistoryItem[] => {
  try {
    ...
    const activeCount = downloads.filter((d) => d.status === "queued" || d.status === "downloading" || d.status === "paused").length;
    const historyCount = downloads.filter((d) => d.status === "completed" || d.status === "failed" || d.status === "cancelled").length;
