...
const downloadFolderControllers = new Map<string, { downloadId: string; cancelRequested: boolean; cancelReason?: 'failed' | 'cancelled' }>();
...
export const startFolderDownload = async (downloadId: string, remotePath: string, localPath: string, folderName: string) => {
  ...
  try {
    ...
    console.log('[Folder Download] Starting file downloads:', { id: downloadId, activeCount: activeFolderDownloads, totalFiles: entries.totalFiles });
    ...
    const errorMessage = (isDisconnected || isErrorDisconnected) ? 'Connection terminated by user' : (isCancelled || isErrorCancelled) ? undefined : err.message;
    const endTime = Date.now();
...
ipcMain.handle('ftp:download-folder', async (event, { remotePath, folderName, downloadId, defaultDownloadPath, duplicateAction, applyToAll, defaultConflictResolution }: {
  ...
  try {
    ...
    if (defaultDownloadPath) {
      ...
      const result = await handleDuplicateFolder(win, defaultPath, folderName, duplicateAction, applyToAll, defaultConflictResolution);
