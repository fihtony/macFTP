...
import { deriveKeyFromPassword, encryptField, decryptField, encryptSite, decryptSite } from "./services/encryption";
...
const createWindow = () => {
  ...
  mainWindow.webContents.on("before-input-event", (event, input) => {
    if (input.key.toLowerCase() === "i" && input.control && input.shift && !input.alt && !input.meta) {
    ...
    } else if (input.key.toLowerCase() === "i" && input.meta && input.alt && !input.control && !input.shift) {
      // Cmd+Option+I (Mac)
      if (mainWindow) {
        if (mainWindow.webContents.isDevToolsOpened()) {
          mainWindow.webContents.closeDevTools();
  ...
  if (finalIconPath) {
    ...
    if (process.platform === "darwin") {
      ...
      // Update About panel icon - use .icns format if available for better macOS compatibility
      const aboutIconPath = getIconPath(true); // Prefer .icns for About panel
      app.setAboutPanelOptions({
        ...
        authors: ["Tony Xu"],
        iconPath: aboutIconPath || finalIconPath, // Use .icns if available, fallback to any icon
      });
    }
  }
};
...
// Note: This will be updated in createWindow() with the actual icon path after window is created
if (process.platform === "darwin") {
  app.setAboutPanelOptions({
    applicationName: "MacFTP",
    applicationVersion: app.getVersion(),
    ...
    credits: "Data is stored locally, credentials are encrypted.",
    authors: ["Tony Xu"],
    // iconPath will be set in createWindow() after icon is found
  });
}
...
app.whenReady().then(async () => {
  ...
  // Update About panel with icon after app is ready and window is created
  // This ensures the icon path is resolved correctly
  if (process.platform === "darwin") {
    const aboutIconPath = getIconPath(true); // Prefer .icns for About panel
    if (aboutIconPath) {
      // Use absolute path for About panel
      const absoluteIconPath = path.resolve(aboutIconPath);
      app.setAboutPanelOptions({
        applicationName: "MacFTP",
        applicationVersion: app.getVersion(),
        copyright: "© 2025 Tony Xu",
        credits: "Data is stored locally, credentials are encrypted.",
        authors: ["Tony Xu"],
        iconPath: absoluteIconPath, // Use absolute path to app icon
      });
      console.log("[Main] About panel icon set to:", absoluteIconPath);
    } else {
      console.warn("[Main] Could not find icon for About panel");
    }
  }
...
// Encryption handlers
ipcMain.handle("encryption:derive-key", (event, args: { password: string; version: string; exportDate: string }) => {
  try {
    const key = deriveKeyFromPassword(args.password, args.version, args.exportDate);
    return { success: true, keyHex: key.toString("hex") };
  } catch (error: any) {
    return { success: false, error: error.message };
  }
});

ipcMain.handle("encryption:encrypt-field", (event, args: { text: string; keyHex: string }) => {
  try {
    const key = Buffer.from(args.keyHex, "hex");
    const encrypted = encryptField(args.text, key);
...
ipcMain.handle("encryption:decrypt-field", (event, args: { encryptedText: string; keyHex: string }) => {
  try {
    const key = Buffer.from(args.keyHex, "hex");
    const decrypted = decryptField(args.encryptedText, key);
...
ipcMain.handle("encryption:encrypt-site", (event, args: { site: any; keyHex: string }) => {
  try {
    const key = Buffer.from(args.keyHex, "hex");
    const encrypted = encryptSite(args.site, key);
    return { success: true, site: encrypted };
  } catch (error: any) {
    return { success: false, error: error.message };
  }
});

ipcMain.handle("encryption:decrypt-site", (event, args: { site: any; keyHex: string }) => {
  try {
    const key = Buffer.from(args.keyHex, "hex");
    const decrypted = decryptSite(args.site, key);
    return { success: true, site: decrypted };
  } catch (error: any) {
    return { success: false, error: error.message };
  }
});
