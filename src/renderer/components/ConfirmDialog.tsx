...
const ConfirmDialog: React.FC<ConfirmDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-background/80 backdrop-blur flex items-center justify-center">
      <div className="bg-card border border-border rounded-lg shadow-xl p-6 w-96 max-w-[calc(100%-2rem)]">
        <div className="flex items-start gap-4 mb-4">
          ...
          >
            <AlertTriangle
              size={24}
              className={variant === "danger" ? "text-red-500" : variant === "warning" ? "text-yellow-500" : "text-blue-500"}
            />
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-semibold mb-2">{title}</h3>
            <p className="text-sm text-muted-foreground whitespace-pre overflow-hidden">{message}</p>
          </div>
        </div>
