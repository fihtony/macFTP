...
const FileExplorer = () => {
  ...
  const submitCreateFolder = async () => {
    ...
    console.log('[User Action] Create folder:', { folderName: sanitized, remotePath: buildRemotePath(sanitized), siteName: currentSite?.name });
  ...
  const executeDelete = async () => {
    ...
    console.log('[User Action] Delete confirmed:', { fileName: file.name, type: file.type === 'd' ? 'folder' : 'file', siteName: currentSite?.name });
    const electron = (window as any).electronAPI;
    if (!electron) return;
    const targetPath = buildRemotePath(file.name);
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!file) return;
    console.log('[User Action] Delete file/folder:', { fileName: file.name, type: file.type === 'd' ? 'folder' : 'file', siteName: currentSite?.name });
  ...
  const handleDownload = async (file: RemoteFile, e: React.MouseEvent) => {
    e.stopPropagation();
    console.log('[User Action] Download file:', { fileName: file.name, remotePath: currentPath === '/' ? `/${file.name}` : `${currentPath}/${file.name}`, siteName: currentSite?.name });
    const electron = (window as any).electronAPI;
    if (!electron) {
    ...
    try {
      ...
      downloadPromise.then((initialResponse: any) => {
      ...
      console.error('[Error] Download failed to start:', { downloadId, fileName: file.name, error: err.message });
      updateDownload(downloadId, {
        status: 'failed',
        error: err.message || 'Unknown error',
        endTime: Date.now()
  ...
  const savePermissions = async () => {
      ...
      console.log('[User Action] Change permissions:', { fileName: propertiesFile.name, permissions: newPermissions, siteName: currentSite?.name });
      const electron = (window as any).electronAPI;
      if (electron) {
