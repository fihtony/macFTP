...
function App() {
  ...
  useEffect(() => {
    ...
    let unsubscribeConnectionLost: (() => void) | undefined;

    if (electron?.onConnectionLost) {
      unsubscribeConnectionLost = electron.onConnectionLost((data: { reason: string; code?: string }) => {
        console.log("[App] Connection lost:", data);

        // Disconnect from UI
        const store = useStore.getState();
        store.setConnected(false);
        store.setCurrentSite(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = electron.onDownloadProgress((payload: DownloadProgressPayload) => {
      ...
      if (currentDownload && (currentDownload.status === "cancelled" || currentDownload.status === "failed")) {
        console.log(
          "[App] Ignoring update for already finished download:",
          payload.id,
          "current status:",
  ...
  const selectedDownload = selectedDownloadId ? downloads.find((d) => d.id === selectedDownloadId) : null;
      ...
      if (
        ...
        if (download.status === "cancelled") {
          ...
          if (!disconnectFailedDownloads.current.has(download.id)) {
            const message = download.isFolder ? `Folder download failed: ${download.fileName}` : `Download failed: ${download.fileName}`;
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-background text-foreground overflow-hidden">
      <TitleBar onToggleDownloadManager={() => setShowDownloadManager(!showDownloadManager)} showDownloadManager={showDownloadManager} />
      ...
      {!showDownloadManager && activeDownload && (
        ...
        >
          <span className="text-sm font-medium">{downloads.filter((d) => d.status === "downloading" || d.status === "queued").length}</span>
        </button>
      )}
